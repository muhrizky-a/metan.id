            ...
            @else
                @foreach($articles as $row)
                    <div class="card mb-3 horizontal-card">
                        <div class="row g-0">
                            <div class="col-md-4">
                                <img src="{{ asset('img/article/'.$row->image_url ) }}" class="img-fluid rounded-start" alt="{{ $row->title}}">
                            </div>

                            <div class="col-md-8">
                                <div class="card-body">
                                    <h5 class="card-title"><a href="{{ route('article.detail', $row->link) }}">{{ $row->title}}</a></h5>
                                    <small class="card-text"><a href="{{ route('category.page', $row->category->link) }}" class="tags">{{ $row->category->name}}</a></small>
                                </div>
                            </div>
                        </div>
                    </div>
